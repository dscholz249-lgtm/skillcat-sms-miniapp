...
async function handleTechnicianInbound({ technician, from, body, media }) {
  ...
  if (!media || media.length === 0) {
    ...
    await sendOut(from, 'Text this number to send photos to your logbook 📸', 'tech-no-media');
    return;
  }
...
async function handleInbound({ from, body, media = [] }) {
  ...
  // Reject unrecognized numbers before any further processing
  const knownCompany = getCompanyByPhone(from);
  const existingSession = getSession(from);
  if (!knownCompany && !existingSession) {
    await sendOut(from, "I'm sorry, I don't recognize this number. Contact SkillCat for more information or help.", 'unrecognized');
    return;
  }
