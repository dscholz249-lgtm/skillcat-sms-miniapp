...
async function handleInbound({ from, body }) {
  ...
  if (stepBefore === 'AWAITING_NAME_SELECTION') {
    ...
    if (resolved) {
      ...
      notifySlack('human_review', { raw: body }, from).catch(() => {});
      deleteSession(from);
...
async function writeIntent(parsed, from, companyId, msgId, session) {
  ...
  if (parsed.intent === 'assign_training') {
    ...
    notifySlack('assign_training', { employee_name: parsed.employee_name, certification_name: certName }, from).catch(() => {});
    deleteSession(from);
  ...
  if (parsed.intent === 'add_employee') {
    ...
    notifySlack('add_employee', { name: newEmp.name, email: newEmp.email ?? null, title: newEmp.title ?? null }, from).catch(() => {});
    deleteSession(from);
