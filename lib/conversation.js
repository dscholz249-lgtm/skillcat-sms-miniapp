...
async function handleInbound({ from, body, media = [] }) {
  ...
  // Phone link flow: unrecognized phone texts their registered email address.
  const emailMatch = /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b/.exec(body.trim());
  if (emailMatch && !getManagerInfoByPhone(from)) {
    await handlePhoneLinkRequest({ from, email: emailMatch[0] });
    return;
  }
