...
async function handleInbound({ from, body, media = [] }) {
  ...
  // Resolve "me" / "myself" to the sender's own name
  if (parsed.employee_name && /^(me|myself)$/i.test(parsed.employee_name.trim())) {
    const selfInfo = getManagerInfoByPhone(from);
    if (selfInfo?.name) parsed.employee_name = selfInfo.name;
  }
