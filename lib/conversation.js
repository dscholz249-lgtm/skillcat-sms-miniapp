...
async function handleInbound({ from, body }) {
  ...
  const { techById, managerByPhone } = await rosterMaps();
  // Used as set_by on the auto-applied field-readiness update so attribution
  // points at the responding manager, not the SMS responder phone.
  const managerId = (managerByPhone.get(from) || {}).id || null;
  ...
  if (session.step === 'AWAITING_SELECTION') {
    return handleSelection(session, from, parsed, pending, techById, managerId);
  }
  if (session.step === 'AWAITING_READINESS') {
    return handleReadiness(session, from, parsed, pending, techById, managerId);
  }
  if (session.step === 'AWAITING_NOTE') {
    return handleNote(session, from, parsed, body, pending, techById, managerId);
  }
  // Fallback — shouldn't happen.
  upsertSession(session);
}

async function handleSelection(session, from, parsed, pending, techById, managerId) {
  if (!parsed.tech_id) {
  ...
  if (parsed.readiness) {
    ...
    if (parsed.note) {
      await writeAndAdvance(session, from, parsed.note, pending, techById, managerId);
      return;
    }
...
async function handleReadiness(session, from, parsed, pending, techById, managerId) {
  if (!parsed.readiness) {
...
async function handleNote(session, from, parsed, rawText, pending, techById, managerId) {
  ...
  await writeAndAdvance(session, from, note, pending, techById, managerId);
}

async function writeAndAdvance(session, from, note, pending, techById, managerId) {
  ...
  const finishedTechId = finishedTech ? finishedTech.tech_id : null;
  // Snapshot the verdict before we clear session.pending_readiness below —
  // we still need it for the field-readiness PATCH.
  const verdict = session.pending_readiness;

  try {
    await platform.postResult(reviewId, { readiness: verdict, note });
  } catch (e) {
    if (e.status === 409) {
  ...
  // Auto-apply the SMS verdict to the technician's Field Readiness indicator.
  // ready → 'go', not_ready → 'nogo'. The dashboard toggle still wins after
  // the fact — a manager can override. Failures here don't break the closer:
  // the result already wrote successfully and is the canonical record.
  if (finishedTechId && verdict) {
    const newState = verdict === 'ready' ? 'go' : 'nogo';
    try {
      await platform.postFieldReadiness(finishedTechId, {
        state: newState,
        set_by: managerId || from,
      });
    } catch (e) {
      console.error('[field-readiness] update failed', e?.message || e, e?.body);
    }
  }
