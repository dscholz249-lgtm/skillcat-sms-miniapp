...
// Auto-update the Field Readiness state to match an SMS verdict. Per the
// product decision after the initial spec: the SMS Y/N is treated as
// authoritative for the indicator. The dashboard toggle still wins after the
// fact — a manager can override here at any time.
async function postFieldReadiness(technicianId, { state, set_by }) {
  return call('PATCH', `/api/technicians/${encodeURIComponent(technicianId)}/field-readiness`, {
    body: { state, set_by },
  });
}

module.exports = { getRoster, postReviewBatch, getReview, postResult, postFieldReadiness };
