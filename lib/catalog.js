const SUPPLEMENT = require('./catalog-supplement');
...
async function getCatalog() {
  if (!_cache.length || Date.now() - _cacheTime >= CACHE_TTL) {
    try {
  ...
  // Always merge supplement, deduped by name
  const liveNames = new Set(_cache.map(c => c.name.toLowerCase()));
  const extras = SUPPLEMENT.filter(c => !liveNames.has(c.name.toLowerCase()));
  return [..._cache, ...extras];
}

function searchCatalog(catalog, query) {
  if (!query || !catalog.length) return [];
  const terms = query.toLowerCase().split(/\s+/).filter(t => t.length > 2 || /^\d+$/.test(t));
...
function fuzzyMatchCourse(catalog, name) {
  ...
  const terms = lower.split(/\s+/).filter(t => t.length > 2 || /^\d+$/.test(t));
  if (!terms.length) return [];
