    ...
    for (const recipient of queued) {
      try {
        ...
        if (Number(code) === TWILIO_OPTED_OUT) {
          ...
          // This may be the first we hear of an opt-out Twilio swallowed at the
          // carrier level, so the notification belongs here too.
          notifyOptOut(recipient.phone).catch(
            e => console.error('[opt-out] notify failed', e.message));
        }
      }
